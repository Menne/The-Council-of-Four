...
public class BuildByPermitTile implements MainAction {
	...
	public boolean executeAction(Game game) throws NullPointerException {
		...
		if (!checkPermitTileContainsCity()) {
			game.notifyObserver(new ErrorServerNotify("It seems that the permit tile you selected doesn't contain the city in which you want to build!. Try again or choose another action", 
					Arrays.asList(game.getCurrentPlayer())));
			return false;
		}	
